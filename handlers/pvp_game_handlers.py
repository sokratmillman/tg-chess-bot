...
from firebase.db_handlers import (
    database, handle_game_reject, handle_game_request,
    handle_game_start, handle_game_finish,
)

from handlers.state import ChessState
from keyboards.accept_invite import (
    accept_invite_kb, AGREE_TEXT, DISAGREE_TEXT,
)

from libs.draw_board import draw_board, send_board
from bot import bot, dp
from texts import (CANCEL_INVITE_TEXT, DRAW_TEXT,
    INVALID_MOVE_TEXT, INVITE_REJECTED_TEXT,
    NOT_YOUR_TURN_TEXT, GAME_STARTED_TEXT,
    USER_MOVE_TEXT, WAIT_OPPONENT_MOVE_TEXT,
    WAITING_INVITE_TEXT, WIN_TEXT, WRONG_ID_TEXT,
)
...
async def start_pvp_game(message: types.Message, state: FSMContext):
    try:
        ...
        await message.answer('''Please, provide partner id. Example: `/start_pvp_game 1235`''',
            parse_mode="Markdown")
        return
    ...
    try:
        ...
        await bot.send_message(partner_id,
            f'Let\'s play with {message.from_user.username}!',
            reply_markup=accept_invite_kb)
...
async def waiting_handler(message: types.Message, state: FSMContext):
    if message.text == AGREE_TEXT:
        ...
        player = database.get_user(message.from_user.id)
        partner_id = player['status'].split(' ')[-1]
...
async def game_handler(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        opp_board_caption = ''
        if end_text == '':
            my_board_caption = USER_MOVE_TEXT.format(message.from_user.username, message.text)
            opp_board_caption = WAIT_OPPONENT_MOVE_TEXT
        else:
            my_board_caption = end_text
            opp_board_caption = end_text

        await send_board(bot, partner_id, board, my_board_caption)
        await draw_board(message, board, opp_board_caption)
    except Exception:
        await message.answer(INVALID_MOVE_TEXT)


def register_pvp_game_handlers(dispatcher: Dispatcher):
    dispatcher.register_message_handler(start_pvp_game, commands="start_pvp_game")
    dispatcher.register_message_handler(waiting_handler, state=ChessState.WAITING)
    dispatcher.register_message_handler(game_handler, state=ChessState.PVP_GAME)
